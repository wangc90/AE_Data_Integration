...
def NonOrtho_AE_retraining(model, model_folder_path, RNA_df_path, miRNA_df_path, hyper_dict):
...
if __name__ == '__main__':
    ...
    for group in [2, 3, 4, 5]:
        for prop_diff in [0.2, 0.4, 0.6, 0.8, 1]:
            ...
            hyper_dict = {}
            with open(f'/home/wangc90/Data_integration/simulation_model_outputs/model_selection_outputs/NonOrtho_AE/optuna/optuna_{group}_groups_{prop_diff}/optuna.txt') as f:
                for _ in range(7):
                    next(f)
                for line in f:
            ...
            model_folder_path = f'/home/wangc90/Data_integration/simulation_model_outputs/model_retraining_outputs/NonOrtho_AE/models/retrained_{group}_groups_{prop_diff}_diff'
            if not os.path.exists(model_folder_path):
                print('Create folder to store retrained models')
                os.makedirs(model_folder_path)

            NonOrtho_AE_retraining(model=SS_AE,
