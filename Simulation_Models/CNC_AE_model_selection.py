...
class CNC_Decoder(nn.Module):
    ...
    def __init__(self, trial):
        ...
        self.s1_input_dim = CNC_Encoder(trial).s1_input_dim
        self.s2_input_dim = CNC_Encoder(trial).s2_input_dim

        self.common_embed_dim = CNC_Encoder(trial).common_embed_dim
...
class CNC_AE(nn.Module):
    def __init__(self, trial):
        ...
        self.encoder = CNC_Encoder(trial)

        self.decoder = CNC_Decoder(trial)
...
def CNC_AE_model_selection(num_trial, val_loss_folder_path, optuna_folder_path, RNA_df_path, miRNA_df_path):
    ...
    if np.alltrue(RNA_df.index == miRNA_df.index):
        ...
        study.optimize(Objective_CV(cv=5, model=CNC_AE,
...
if __name__ == '__main__':
    ...
    for group in [2, 3, 4, 5]:
        for prop_diff in [0.2, 0.4, 0.6, 0.8, 1]:
            ...
            val_loss_folder_path = f'/home/wangc90/Data_integration/simulation_model_outputs/model_selection_outputs/CNC_AE/val_loss/val_{group}_groups_{prop_diff}_loss'
            optuna_folder_path = f'/home/wangc90/Data_integration/simulation_model_outputs/model_selection_outputs/CNC_AE/optuna/optuna_{group}_groups_{prop_diff}'
            ...
            CNC_AE_model_selection(num_trial=50, val_loss_folder_path=val_loss_folder_path,
