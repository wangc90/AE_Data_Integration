...
class X_AE_Decoder(nn.Module):
    ...
    def __init__(self, trial):
        ...
        self.s1_input_dim = X_AE_Encoder(trial).s1_input_dim
        self.s2_input_dim = X_AE_Encoder(trial).s2_input_dim
        self.common_embed_dim = X_AE_Encoder(trial).common_embed_dim
...
class X_AE(nn.Module):
    def __init__(self, trial):
        ...
        self.encoder = X_AE_Encoder(trial)

        self.decoder = X_AE_Decoder(trial)
...
def X_AE_model_selection(num_trial, val_loss_folder_path, optuna_folder_path, RNA_df_path, miRNA_df_path):
    ...
    if np.alltrue(RNA_df.index == miRNA_df.index):
        ...
        study.optimize(Objective_CV(cv=5, model=X_AE,
...
if __name__ == '__main__':
    ...
    for group in [2, 3, 4, 5]:
        for prop_diff in [0.2, 0.4, 0.6, 0.8, 1]:
            ...
            val_loss_folder_path = f'/home/wangc90/Data_integration/simulation_model_outputs/model_selection_outputs/X_AE/val_loss/val_{group}_groups_{prop_diff}_loss'
            optuna_folder_path = f'/home/wangc90/Data_integration/simulation_model_outputs/model_selection_outputs/X_AE/optuna/optuna_{group}_groups_{prop_diff}'
            ...
            X_AE_model_selection(num_trial=50, val_loss_folder_path=val_loss_folder_path,
