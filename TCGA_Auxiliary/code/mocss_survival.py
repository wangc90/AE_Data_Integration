...
def main():
    ...
    # Load BRCA_data
    view1_data = pd.read_csv('../../data/Survival_data/melanoma/1_all.csv', header=None)
    print(view1_data.shape)
    view1_size = view1_data.shape[1]
    view2_data = pd.read_csv('../../data/Survival_data/melanoma/2_all.csv', header=None)
    print(view2_data.shape)
    view2_size = view2_data.shape[1]
    view3_data = pd.read_csv('../../data/Survival_data/melanoma/3_all.csv', header=None)
    print(view3_data.shape)
    view3_size = view3_data.shape[1]
    ...
    # Build Model
    model = SharedAndSpecificEmbedding(view_size=[view1_size, view2_size, view3_size], n_units_1=[512, 256, 128, 32], n_units_2=[512, 256, 128, 32], n_units_3=[512, 256, 128, 32],mlp_size=[32,8])
    ...
    for epoch in range(EPOCH):
        ...
        for iteration_index, train_batch in enumerate(train_loader):
            ...
            view1_train_data = train_data[:, :view1_size]
            # print(view1_train_data.shape)
            view1_train_data = torch.tensor(view1_train_data).clone().detach()
            # print(view1_train_data.shape)
            view2_train_data = train_data[:, view1_size:view1_size + view2_size]
            # print(view2_train_data.shape)
            view2_train_data = torch.tensor(view2_train_data).clone().detach()
            # print(view2_train_data.shape)
            view3_train_data = train_data[:, view1_size + view2_size:view1_size + view2_size + view3_size]
        ...
        #train metrics
        view1_all = torch.tensor(view_train_concatenate[:, :view1_size]).clone().detach().float()
        view2_all = torch.tensor(view_train_concatenate[:, view1_size:view1_size + view2_size]).clone().detach().float()
        view3_all = torch.tensor(view_train_concatenate[:, view1_size + view2_size:view1_size + view2_size + view3_size ]).clone().detach().float()
        ...
        print('[Epoch: %3d/%3d] Training Loss: %f' % (epoch + 1, EPOCH, train_loss_[epoch]))
    ...
    # final_embedding = view_shared_common
    final_embedding = final_embedding.detach().numpy()
    np.savetxt('../../data/Survival_data/melanoma/final_embedding.txt', final_embedding)
    for i in range(3):
        km = KMeans(n_clusters=n_clusters + i, init='k-means++')
        y_pred1 = km.fit_predict(final_embedding)
        np.savetxt('../../data/Survival_data/melanoma/cluster_label_{number}.txt'.format(number=n_clusters + i), y_pred1)
        y_pred1 = pd.DataFrame(y_pred1)
        file = open('../../data/Survival_data/melanoma/cluster_label_{number}.csv'.format(number=n_clusters + i), 'w')
        y_pred1.to_csv(file, header=0,index=None)
    # nmi_, ari_, f_score_, acc_ = evaluation.evaluate(y_true, y_pred1)
    # print('\n' + ' ' * 8 + '|==>  nmi: %.4f,  ari: %.4f  <==|' % (nmi(y_true, y_pred1), ari(y_true, y_pred1)))
